...
def segment_background_and_tissue(image_array_3d, axis_first=True, coef=1.0):
    ...
    image_array_3d = (image_array_3d - image_array_3d.min()) / \
        (image_array_3d.max() - image_array_3d.min())
    ...
    for idx in range(z_num):
        ...
        if axis_first:
            ...
            image_slice = image_array_3d[..., idx]
        thresh, _ = cv2.threshold(image_slice, 0, 255, cv2.THRESH_OTSU)
        brain_mask_slice = image_slice > thresh * coef
        for idx in range(3):
            brain_mask_slice = binary_erosion(brain_mask_slice)
        brain_mask_slice = remove_small_objects(
            brain_mask_slice, min_size=h * w * 0.1)
        for idx in range(3):
    ...
    brain_mask = remove_small_holes(
        brain_mask, area_threshold=h * w * z_num * 0.25)
