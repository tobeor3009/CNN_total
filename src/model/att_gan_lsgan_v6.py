...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        feature_alpha = tf.random.uniform((batch_size, self.feature_h, self.feature_w, 1),
                                          0, 1)
        feature_alpha = tf.cast(feature_alpha >= 0.5, tf.float32)
        ...
        with tf.GradientTape(persistent=False) as disc_tape:
            ...
            _, label_real_x = self.discriminator(real_x,
                                                 training=True)
            ...
            disc_fake_blend, _ = self.discriminator(blend_fake_image,
                                                    training=True)
            disc_real_blend_loss = self.disc_loss_fn(feature_real_alpha,
                                                     disc_real_blend)
            disc_blend_loss = self.disc_loss_fn(feature_alpha,
                                                disc_blend)
            disc_fake_blend_loss = self.disc_loss_fn(feature_fake_alpha,
                                                     disc_fake_blend)

            disc_loss = (disc_real_blend_loss +
                         disc_blend_loss + disc_fake_blend_loss) / 3
        ...
        return {
            ...
            "disc_fake_blend_loss": disc_blend_loss,
