...
def Deeplabv3(weights=None, input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='xception',
    ...
    current_available_activation = {'softmax', 'sigmoid', 'tanh'}
    if activation in current_available_activation:
        x = tf.keras.layers.Activation(activation)(x)
    elif activation is not None:
        print(
            f"current_available_activation is {current_available_activation}.\nyour activation: {activation}")
...
def Deeplabv3Classification(input_tensor=None, input_shape=(512, 512, 3), classes=21, backbone='xception',
                            OS=16, alpha=1., activation=None, grad_cam=False):
    """ Instantiates the Deeplabv3+ architecture
    Optionally loads weights pre-trained
    on PASCAL VOC or Cityscapes. This model is available for TensorFlow only.
    # Arguments
    ...
    x = layers.Dense(classes, activation=activation,
                     dtype=dense_dtype)(x)
    # Ensure that the model takes into account
    # any potential predecessors of `input_tensor`.
    if input_tensor is not None:
