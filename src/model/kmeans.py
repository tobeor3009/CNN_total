...
def run_sample(gpuid, n_clusters, num, max_iter, use_custom_kernel, output):
...
def run(X_train, gpuid, n_clusters, max_iter, use_custom_kernel=False, output="temp.png"):
    ...
    with cupy.cuda.Device(gpuid):
        ...
        if output is not None:
            index = numpy.random.choice(len(X_train), 300, replace=False)
            draw(X_train[index].get(), n_clusters, centers.get(),
                 pred[index].get(), output)
...
if __name__ == '__main__':
    ...
    run_sample(args.gpu_id, args.n_clusters, args.num, args.max_iter,
