...
def window_reverse(windows, window_size, H, W, C):
    ...
    patch_num_H = H // window_size
    patch_num_W = W // window_size
    x = tf.reshape(windows, shape=(-1, patch_num_H,
                   patch_num_W, window_size, window_size, C))
...
class WindowAttention(tf.keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        ...
        if mask is not None:
            nW = mask.get_shape()[0]
            mask_float = tf.cast(tf.expand_dims(
                tf.expand_dims(mask, axis=1), axis=0), tf.float32)
            attn = tf.reshape(
