...
def swin_class_disc_2d_base(input_tensor, filter_num_begin, depth, stack_num_down, stack_num_up,
                            patch_size, stride_mode, num_heads, window_size, num_mlp, act="gelu", shift_window=True,
                            swin_v2=False, name='unet'):
    '''
    The base of Swin-UNET.
...
def get_swin_class_gen_2d(input_shape, class_num, last_channel_num,
    ...
    # Base architecture
    X, FEATURE_X = swin_class_gen_2d_base(IN, CLASS, filter_num_begin, depth, stack_num_down, stack_num_up,
    ...
    # Model configuration
    model = Model(inputs=[IN, CLASS], outputs=[OUT, FEATURE_X])
    return model
...
def get_swin_class_disc_2d(input_shape, last_channel_num,
                           ...
                           patch_size, stride_mode, num_heads, window_size, num_mlp,
                           act="gelu", disc_act="sigmoid", shift_window=True, swin_v2=False):
    IN = layers.Input(input_shape)
    ENCODED_X, FEATRUE_X = swin_class_disc_2d_base(IN, filter_num_begin, depth, stack_num_down, stack_num_up,
                                                   patch_size, stride_mode, num_heads, window_size, num_mlp, act=act,
                                                   shift_window=shift_window, swin_v2=swin_v2, name='unet')
