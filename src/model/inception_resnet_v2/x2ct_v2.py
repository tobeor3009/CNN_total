...
def get_x2ct_model_ap_lat_v10(xray_shape, ct_series_shape,
    ...
    model, skip_connect_name_list = HighWayResnet2D(input_shape=(xray_shape[0], xray_shape[1], xray_shape[2]),
                                                    block_size=block_size,
                                                    num_downsample=num_downsample,
                                                    padding="same",
                                                    base_act=base_act,
    ...
    # lat_output.shape: [B, 16, 16, 1536]
    _, H, W, C = backend.int_shape(output)
    attn_num_head = 8
    attn_dim = C // attn_num_head

    decoded = layers.Reshape((H * W, C))(output)
    ...
    ap_decoded = layers.Reshape((ct_start_channel,
                                 ct_start_channel,
                                 ct_start_channel,
                                 block_size * ct_start_channel * 4))(ap_decoded)
