...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        label_x = tf.tile(label_x[:, None, None, :],
                          (1, self.image_h, self.image_w, 1))
        label_y = tf.tile(label_y[:, None, None, :],
                          (1, self.image_h, self.image_w, 1))
        ...
        image_fake_alpha = tf.image.resize(feature_fake_alpha, (self.image_h, self.image_w),
                                           method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
        label_real_alpha = (label_x * image_real_alpha +
                            label_y * 1 - image_real_alpha)
        label_middle_alpha = (label_x * image_middle_alpha +
                              label_y * 1 - image_middle_alpha)
        label_fake_alpha = (label_x * image_fake_alpha +
                            label_y * 1 - image_fake_alpha)
        # =================================================================================== #
        ...
        with tf.GradientTape(persistent=False) as disc_tape:
            ...
            disc_real, label_real_x = self.discriminator([real_x, label_x],
                                                         training=True)
            disc_real_blend, _ = self.discriminator([blend_real_image, label_real_alpha],
                                                    training=True)
            disc_middle_blend, _ = self.discriminator([blend_middle_image, label_middle_alpha],
                                                      training=True)
            disc_fake_blend, _ = self.discriminator([blend_fake_image, label_fake_alpha],
                                                    training=True)
            disc_fake, _ = self.discriminator([fake_y, label_y],
                                              training=True)
        ...
        with tf.GradientTape(persistent=False) as gen_tape:
            ...
            disc_middle_blend_loss = backend.mean(self.disc_loss_fn(1 - feature_middle_alpha,
                                                                    disc_middle_blend))

            gen_disc_loss = (gen_fake_y_disc_loss + disc_middle_blend_loss) / 2
