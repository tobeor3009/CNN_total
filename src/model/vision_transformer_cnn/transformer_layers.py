...
class PatchMerging(layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, norm="layer", swin_v2=False, name=''):
        ...
        self.linear_trans = layers.Conv2D(2 * embed_dim, kernel_size=1, padding="same",
                                          use_bias=False,
    ...
    def call(self, x):
        ...
        assert h == H and h == W, 'input feature has wrong size'
...
class PatchMerging3D(layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, include_3d=False, norm="layer", swin_v2=False, name=''):
        ...
        # A linear transform that doubles the channels
        self.linear_trans = layers.Conv3D(2 * embed_dim, kernel_size=1, padding="same",
                                          use_bias=False,
                                          name='{}_linear_trans'.format(name))
        self.norm = get_norm_layer(norm)
    ...
    def call(self, x):
        ...
        assert z == Z and h == H and w == W, 'input feature has wrong size'
...
class PatchExpanding(layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate,
        ...
        self.upsample_linear_trans = layers.Conv2D(embed_dim // 2, kernel_size=3,
                                                   padding="same", use_bias=False,
                                                   name='{}_upsample_linear_trans'.format(name))
        self.pixel_shuffle_linear_trans = layers.Conv2D(upsample_rate * embed_dim, kernel_size=3,
                                                        padding="same", use_bias=False,
...
class PatchExpanding3D(layers.Layer):
    ...
    def __init__(self, num_patch, embed_dim, upsample_rate, return_vector=True, norm="layer", swin_v2=False, name=''):
        ...
        self.linear_trans_1 = layers.Conv3D(embed_dim // 2,
                                            kernel_size=3, use_bias=False, name='{}_upsample_linear_trans'.format(name))
        self.linear_trans_2 = layers.Conv3D(embed_dim // 2,
                                            kernel_size=3,
                                            use_bias=False,
                                            name='{}_concat_linear_trans'.format(name))
    ...
    def call(self, x):
        ...
        assert h == H and w == W and z == Z, 'input feature has wrong size'

        upsample = self.linear_trans_1(x)
        upsample = self.upsample_layer(upsample)
        if self.swin_v2:
            x = self.linear_trans_2(x)
            x = self.norm(x)
        else:
            x = self.norm(x)
            x = self.linear_trans_2(x)
