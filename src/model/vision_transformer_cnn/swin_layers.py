...
from .util_layers import drop_path_2d, drop_path_3d, get_norm_layer, get_act_layer
...
class ConvProj2D(layers.Layer):
    def __init__(self, filter_num, act="gelu", drop=0., name=''):
        super().__init__()
        self.conv_1 = layers.Conv2D(filter_num[0], kernel_size=3, padding="same",
                                    name=f'{name}_conv_0')
        self.conv_2 = layers.Conv2D(filter_num[1], kernel_size=3, padding="same",
                                    name=f'{name}_conv_1')
        self.drop = Dropout(drop)
        self.activation = get_act_layer(act)
...
class ConvProj3D(layers.Layer):
    def __init__(self, filter_num, act="gelu", drop=0., name=''):
        super().__init__()
        self.conv_1 = layers.Conv3D(filter_num[0], kernel_size=3, padding="same",
                                    name=f'{name}_conv_0')
        self.conv_2 = layers.Conv3D(filter_num[1], kernel_size=3, padding="same",
                                    name=f'{name}_conv_1')
        self.drop = Dropout(drop)
        self.activation = get_act_layer(act)
...
class WindowAttention(layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0, proj_drop=0., swin_v2=False, name=''):
        ...
        self.prefix = name
        # Layers
        self.conv_q = layers.Conv2D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_q'.format(self.prefix))
        self.conv_k = layers.Conv2D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_k'.format(self.prefix))
        self.conv_v = layers.Conv2D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_v'.format(self.prefix))
        self.attn_drop = Dropout(attn_drop)
        self.proj = layers.Conv2D(dim, kernel_size=3, padding="same",
                                  name='{}_attn_proj'.format(self.prefix))
        self.proj_drop = Dropout(proj_drop)

    def build(self, current_shape):
        H, W, C = current_shape[1:]
        self.reshape_layer_2d_1d = layers.Reshape((H * W, C))
        self.reshape_layer_1d_2d = layers.Reshape((H, W, self.dim))
        # zero initialization
    ...
    def call(self, current, hidden, mask=None):

        # Get input tensor static shape
        _, H, W, C = current.get_shape().as_list()
        N = H * W

        q = self.conv_q(current)
        k = self.conv_k(hidden)
        v = self.conv_v(hidden)
        ...
        q = self.reshape_2d_1d(q, N)
        k = self.reshape_2d_1d(k, N)
        v = self.reshape_2d_1d(v, N)
        ...
        # Merge qkv vectors
        x_qkv = (attn @ v)  # [B num_heads H*W C]
        x_qkv = tf.transpose(x_qkv, perm=(0, 2, 1, 3))
        x_qkv = tf.reshape(x_qkv, shape=(-1, H, W, C))
        ...
        x_qkv = self.reshape_layer_1d_2d(x_qkv)
        return x_qkv
    ...
    def reshape_2d_1d(self, q_k_v, N):
        q_k_v = self.reshape_layer_2d_1d(q_k_v)
        q_k_v = tf.reshape(q_k_v, shape=(-1, N, self.num_heads, self.head_dim))
        q_k_v = tf.transpose(q_k_v, perm=(0, 2, 1, 3))
        return q_k_v
...
class WindowAttention3D(layers.Layer):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0, proj_drop=0., swin_v2=False, name=''):
        ...
        self.swin_v2 = swin_v2
        self.prefix = name
        self.conv_q = layers.Conv3D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_q'.format(self.prefix))
        self.conv_k = layers.Conv3D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_k'.format(self.prefix))
        self.conv_v = layers.Conv3D(dim, kernel_size=3, padding="same",
                                    name='{}_attn_v'.format(self.prefix))
        # Layers
        self.attn_drop = Dropout(attn_drop)
        self.proj = layers.Conv3D(dim, kernel_size=3, padding="same",
                                  name='{}_attn_proj'.format(self.prefix))
        self.proj_drop = Dropout(proj_drop)

    def build(self, current_shape):
        Z, H, W, C = current_shape[1:]
        self.reshape_layer_3d_1d = layers.Reshape((Z * H * W, C))
        self.reshape_layer_1d_3d = layers.Reshape((Z, H, W, self.dim))
    ...
    def call(self, current, hidden, mask=None):
        ...
        # Get input tensor static shape
        _, Z, H, W, C = current.get_shape().as_list()
        N = Z * H * W
        ...
        q = self.reshape_3d_1d(q, N)
        k = self.reshape_3d_1d(k, N)
        v = self.reshape_3d_1d(v, N)
        if self.swin_v2:
            scale = tf.minimum(self.scale, np.log(1. / .01))
            scale = tf.exp(scale)
            q, _ = tf.linalg.normalize(q, axis=-1)
        ...
        x_qkv = (attn @ v)
        x_qkv = tf.transpose(x_qkv, perm=(0, 2, 1, 3))
        x_qkv = tf.reshape(x_qkv, shape=(-1, Z, H, W, C))
        ...
        # Dropout after projection
        x_qkv = self.proj_drop(x_qkv)
        x_qkv = self.reshape_layer_1d_3d(x_qkv)
        return x_qkv
    ...
    def reshape_3d_1d(self, q_k_v, N):
        q_k_v = self.reshape_layer_3d_1d(q_k_v)
        q_k_v = tf.reshape(q_k_v, shape=(-1, N, self.num_heads, self.head_dim))
        q_k_v = tf.transpose(q_k_v, perm=(0, 2, 1, 3))
        return q_k_v
...
class SwinTransformerBlock(layers.Layer):
    def __init__(self, dim, num_patch, num_heads, window_size=7, shift_size=0, num_mlp=1024, act="gelu", norm="layer",
        ...
        self.drop_path = drop_path_2d(drop_path_prob)
        self.norm2 = get_norm_layer(norm, name='{}_norm2'.format(self.prefix))
        self.conv_proj = ConvProj2D([num_mlp, dim], act=act,
    ...
    def call(self, x):
        ...
        assert h == H and h == W, 'Number of patches before and after Swin-MSA are mismatched.'
        ...
        attn_windows = self.attn(x_windows, x_windows, mask=self.attn_mask)
...
class SwinTransformerContextBlock(layers.Layer):
    def __init__(self, dim, num_patch, num_heads, window_size=7, shift_size=0, num_mlp=1024, act="gelu", norm="layer",
                 qkv_bias=True, qk_scale=None, mlp_drop=0, attn_drop=0, proj_drop=0, drop_path_prob=0, swin_v2=False, name=''):
        super().__init__()
...
class SwinTransformerBlock3D(layers.Layer):
    def __init__(self, dim, num_patch, num_heads, window_size=7, shift_size=0, num_mlp=1024, act="gelu", norm="layer",
        ...
        self.attn = WindowAttention3D(dim, window_size=self.window_size, num_heads=num_heads,
                                      ...
                                      attn_drop=attn_drop, proj_drop=proj_drop,
                                      swin_v2=swin_v2, name=self.prefix)
        self.drop_path = drop_path_3d(drop_path_prob)
        self.norm2 = get_norm_layer(norm, name='{}_norm2'.format(self.prefix))
        self.conv_proj = ConvProj3D([num_mlp, dim], act=act,
                                    drop=mlp_drop, name=self.prefix)
    ...
    def call(self, x):
        z, h, w = self.num_patch
        B, Z, H, W, C = x.get_shape().as_list()
        num_window_elements = np.prod(self.window_size)
        # Checking num_path and tensor sizes
        assert z == Z and h == H and w == W, 'Number of patches before and after Swin-MSA are mismatched.'
