...
def get_diff_label(label_tensor):
    ...
    label_tensor = backend.random_uniform(
...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            disc_total_loss = (disc_real_loss + disc_fake_loss + disc_gp * 10 +
        ...
        with tf.GradientTape(persistent=False) as gen_tape:
            ...
            gen_disc_blend_loss = backend.mean(
                self.disc_real_loss_fn(disc_blend))
            gen_disc_loss = (gen_fake_y_disc_loss + gen_disc_blend_loss) / 2
