...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            disc_arrest_gen_loss = self.disc_arrest_gen_loss(disc_real_x,
                                                             disc_fake_y)
            disc_arrest_gen_loss = backend.mean(disc_arrest_gen_loss)
            disc_gp = gradient_penalty(self.discriminator, real_x, fake_y,
                                       mode="2d")
            disc_gp = backend.mean(disc_gp)

            disc_total_loss = (disc_class_loss * self.disc_class_loss_coef + disc_arrest_gen_loss +
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            gen_total_loss = gen_class_loss * self.gen_class_loss_coef + \
                gen_disc_loss * self.gen_disc_loss_coef + \
