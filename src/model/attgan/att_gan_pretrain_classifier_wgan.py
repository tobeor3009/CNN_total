...
def interpolate(label_shape, real, fake, real_label, fake_label, mode="2d"):
    alpha = tf.random.normal([label_shape[0]], 0.0, 1.0)
    if mode == "2d":
        image_alpha = alpha[:, None, None, None]
    elif mode == "3d":
        image_alpha = alpha[:, None, None, None, None]
    label_alpha = alpha[:, None]
    inter = real + image_alpha * (fake - real)
    inter_label = real_label + label_alpha * (fake_label - real_label)
    return inter, inter_label


@tf.function
def gradient_penalty(disc, real, fake, real_label, fake_label, gp_weight,
                     mode="2d", smooth=1e-7):
    label_shape = tf.shape(real)
    inter, inter_label = interpolate(label_shape, real, fake,
                                     real_label, fake_label, mode=mode)
    with tf.GradientTape() as gp_tape:
        gp_tape.watch(inter)
        # 1. Get the discriminator output for this interpolated image.
        # disc.output = [validity, class]
        pred = disc([inter, inter_label], training=True)
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            disc_gp = gradient_penalty(self.discriminator, real_x, fake_y,
                                       label_x, label_y, self.lambda_gp, mode="2d")
            disc_total_loss = (disc_real_x_loss + disc_fake_y_loss +
