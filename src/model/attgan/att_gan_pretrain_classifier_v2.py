...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        real_disc = backend.concatenate([real_label_x, real_label_x], axis=-1)
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            fake_disc = backend.concatenate(
                [real_label_x, fake_label_y], axis=-1)
            # Discriminator output
            disc_real_x = self.discriminator(real_disc,
                                             training=True)
            disc_fake_y = self.discriminator(fake_disc,
                                             training=True)
            disc_real_x_loss = self.disc_real_loss_fn(disc_real_x)
            disc_fake_y_loss = self.disc_fake_loss_fn(disc_fake_y)
            disc_total_loss = (disc_real_x_loss + disc_fake_y_loss) / 2
