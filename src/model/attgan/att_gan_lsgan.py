...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        real_disc = backend.concatenate([real_x, real_x], axis=-1)
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            fake_disc = backend.concatenate([real_x, fake_y], axis=-1)
            # Discriminator output
            disc_real_x, label_pred_real_x = self.discriminator(real_disc,
                                                                training=True)
            disc_fake_y, _ = self.discriminator(fake_disc,
                                                training=True)
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            same_x = self.generator([real_x, label_x],
                                    training=True)
            # Discriminator output
            disc_fake_y, label_pred_fake_y = self.discriminator(fake_disc)
