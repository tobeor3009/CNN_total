...
class ATTGan(Model):
    ...
    def compile(
        ...
        self.style_class_coef = style_class_coef
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=False) as disc_tape:
            ...
            disc_real_x, label_real_x = self.discriminator([real_x, label_x],
                                                           training=True)
            disc_real_wrong_x, label_real_wrong_x = self.discriminator([real_x, label_y],
                                                                       training=True)
            disc_fake_y, label_fake_y = self.discriminator([fake_y, label_y],
                                                           training=True)
            disc_fake_wrong_y, label_fake_wrong_y = self.discriminator([fake_y, label_x],
                                                                       training=True)
            ...
            disc_fake_wrong_y_loss = backend.mean(
                self.disc_fake_loss_fn(disc_fake_wrong_y))
            disc_loss = (disc_real_x_loss + disc_real_wrong_x_loss +
                         disc_fake_y_loss + disc_fake_wrong_y_loss) / 4
            label_real_x_loss = backend.mean(self.class_loss_fn(label_x,
                                                                label_real_x))
            label_real_wrong_x_loss = backend.mean(self.class_loss_fn(label_x,
                                                                      label_real_wrong_x))
            label_fake_y_loss = backend.mean(self.class_loss_fn(label_y,
                                                                label_fake_y))
            label_fake_wrong_y_loss = backend.mean(self.class_loss_fn(label_y,
                                                                      label_fake_wrong_y))

            disc_label_loss = (label_real_x_loss + label_real_wrong_x_loss +
                               label_fake_y_loss + label_fake_wrong_y_loss) / 4
        ...
        with tf.GradientTape(persistent=False) as gen_tape:
            ...
            disc_fake_y, label_fake_y = self.discriminator([fake_y,
                                                            label_y])
            disc_fake_wrong_y, _ = self.discriminator([fake_y,
                                                       label_x])
            disc_same_x, _ = self.discriminator([same_x,
                                                 label_x])
            label_fake_y_loss = backend.mean(self.class_loss_fn(label_y,
                                                                label_fake_y))
            # label_fake_wrong_y_loss = backend.mean(self.class_loss_fn(label_y,
            #                                                           label_fake_wrong_y))
            # gen_label_loss = (label_fake_y_loss + label_fake_wrong_y_loss) / 2
            gen_label_loss = label_fake_y_loss

            # Generator adverserial loss
            gen_fake_y_disc_loss = backend.mean(
                self.disc_real_loss_fn(disc_fake_y))
            gen_same_x_disc_loss = backend.mean(
                self.disc_real_loss_fn(disc_same_x))
            # gen_fake_wrong_y_disc_loss = backend.mean(
            #     self.disc_fake_loss_fn(disc_fake_wrong_y))
            # gen_disc_loss = (gen_fake_y_disc_loss +
            #                  gen_fake_wrong_y_disc_loss) / 2
            gen_disc_loss = (gen_fake_y_disc_loss + gen_same_x_disc_loss) / 2
        ...
        with tf.GradientTape(persistent=False) as style_tape:
            ...
            fake_y = self.generator([real_x, style_y_from_x])
            disc_fake_y, label_fake_y = self.discriminator([fake_y, label_y])
            disc_fake_wrong_y, label_fake_wrong_y = self.discriminator([
                                                                       fake_y, label_x])
            ...
            label_fake_wrong_y_loss = backend.mean(self.class_loss_fn(label_y,
                                                                      label_fake_wrong_y))
            style_class_loss = (label_fake_y_loss +
                                label_fake_wrong_y_loss) / 2
            style_total_loss = disc_diff_loss + style_class_loss * self.style_class_coef
