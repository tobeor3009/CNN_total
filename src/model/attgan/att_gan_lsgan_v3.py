    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=False) as disc_tape:
            ...
            disc_real_x, label_real_x = self.discriminator([real_x, label_x],
                                                           training=True)
            disc_real_wrong_x, _ = self.discriminator([real_x, label_y],
                                                      training=True)
            disc_fake_y, _ = self.discriminator([fake_y, label_y],
                                                training=True)
            disc_fake_wrong_y, _ = self.discriminator([fake_y, label_x],
                                                      training=True)
            ...
            disc_loss = (disc_real_x_loss * 2.5 + disc_real_wrong_x_loss +
                         disc_fake_y_loss + disc_fake_wrong_y_loss) / 5.5
