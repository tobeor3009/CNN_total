...
# this code line based on
# https://github.com/qubvel/classification_models/blob/a0f006e05485a34ccf871c421279864b0ccd220b/classification_models/models/resnet.py#L6
...
def residual_conv_block(filters, stage, block, strides=(1, 1), attention=None, cut='pre'):
    ...
    def layer(input_tensor):
        ...
        x = layers.BatchNormalization(
            name=bn_name + '1', **bn_params)(input_tensor)
        ...
        if cut == 'pre':
            ...
            shortcut = layers.Conv2D(
                filters, (1, 1), name=sc_name, strides=strides, **conv_params)(x)
        else:
        ...
        x = layers.ZeroPadding2D(padding=(1, 1))(x)
        x = layers.Conv2D(filters, (3, 3), name=conv_name +
                          '2', **conv_params)(x)
...
def residual_bottleneck_block(filters, stage, block, strides=None, attention=None, cut='pre'):
    ...
    def layer(input_tensor):
        ...
        if cut == 'pre':
            ...
            shortcut = layers.Conv2D(
                filters * 4, (1, 1), name=sc_name, strides=strides, **conv_params)(x)
        else:
            raise ValueError('Cut type not in ["pre", "post"]')

        # continue with convolution layers
        x = layers.Conv2D(filters, (1, 1), name=conv_name +
                          '1', **conv_params)(x)
        ...
        x = layers.Conv2D(filters * 4, (1, 1),
                          name=conv_name + '3', **conv_params)(x)
...
def ResNet(model_params, input_shape=None, input_tensor=None, include_top=True,
    ...
    # resnet bottom
    x = layers.BatchNormalization(
        name='bn_data', **no_scale_bn_params)(img_input)
    x = layers.ZeroPadding2D(padding=(3, 3))(x)
    x = layers.Conv2D(init_filters, (7, 7), strides=(2, 2),
...
resnet50_skip_connect_name = ('stage4_unit1_relu1', 'stage3_unit1_relu1',
                              'stage2_unit1_relu1', 'relu0')
resnet50_skip_connect_shape = [(None, 32, 32, 1024), (None, 64, 64, 512),
