...
class Pix2PixGan(Model):
    ...
    def set_gen_metric(self, fake_data):
        self.gen_fake_metric.update_state(tf.ones_like(fake_data) >= 0,
                                          fake_data >= 0)

    def set_disc_metric(self, real_data, fake_data):
        self.disc_real_metric.update_state(tf.ones_like(real_data) >= 0,
                                           real_data >= 0)
        self.disc_fake_metric.update_state(tf.ones_like(fake_data) >= 0,
                                           fake_data < 0)
    ...
    def disc_3d_to_batch_size(self, disc_3d, slice_num):
        ...
        batch_disc_3d = backend.mean(batch_disc_3d)
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as disc_tape:
            ...
            disc_arrest_gen_loss = backend.mean(self.disc_arrest_gen_loss(disc_real_y,
                                                                          disc_fake_y))
            # disc_arrest_gen_loss = self.disc_3d_to_batch_size(disc_arrest_gen_loss,
            #                                                   CHECK_SLICE_NUM)
            disc_gp = backend.mean(gradient_penalty(self.discriminator,
                                                    slice_real_y, slice_fake_y, mode="2d"))
            disc_loss = disc_arrest_gen_loss + disc_gp * self.lambda_gp
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            gen_decive_disc_loss = backend.mean(
                self.gen_decive_disc_loss(gen_disc_fake_y))
            # gen_decive_disc_loss = self.disc_3d_to_batch_size(gen_decive_disc_loss,
            #                                                   CHECK_SLICE_NUM)
            total_generator_loss = gen_loss_in_real_y * self.lambda_image + \
                gen_decive_disc_loss * self.lambda_disc
