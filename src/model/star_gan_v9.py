    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=False) as disc_tape:
            ...
            disc_real_x, label_predicted_real_x = self.discriminator(real_x,
                                                                     training=True)
            disc_reconstruct_x, label_predicted_reconstruct_x = self.discriminator(reconstruct_x,
                                                                                   training=True)
            disc_fake_y, label_predicted_fake_y = self.discriminator(fake_y,
                                                                     training=True)

            # Compute Discriminator class_loss_x
            disc_class_loss_real_x = self.class_loss_fn(label_x,
                                                        label_predicted_real_x)
            disc_reconstruct_class_loss_x = self.class_loss_fn(label_x,
                                                               label_predicted_reconstruct_x)
            disc_reconstruct_loss_x = self.discriminator_loss_arrest_generator(disc_real_x,
                                                                               disc_reconstruct_x)

            # Compute Discriminator class_loss_y
            disc_fake_class_loss_y = self.class_loss_fn(label_y,
                                                        label_predicted_fake_y)

            # Compute Discriminator wgan_loss_y
            disc_fake_loss_y = self.discriminator_loss_arrest_generator(disc_real_x,
                                                                        disc_fake_y)

            disc_total_loss_x = disc_reconstruct_loss_x + \
                disc_class_loss_real_x + \
                disc_reconstruct_class_loss_x
            disc_total_loss_y = disc_fake_loss_y + disc_fake_class_loss_y
            ...
            disc_same_x, label_predicted_same_x = self.discriminator(same_x,
                                                                     training=True)
            disc_same_class_loss_x = self.class_loss_fn(label_x,
                                                        label_predicted_same_x)
            # Compute Discriminator wgan_loss_y
            disc_same_loss_x = self.discriminator_loss_arrest_generator(disc_real_x,
                                                                        disc_same_x)
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            # Discriminator output
            disc_fake_y, label_predicted_fake_y = self.discriminator(fake_y)
            disc_reconstruct_x, label_predicted_reconstruct_x = self.discriminator(real_x,
                                                                                   reconstruct_x)
            disc_same_x, label_predicted_same_x = self.discriminator(same_x)
            # Generator adverserial loss
            gen_reconstruct_disc_loss_x = self.generator_loss_deceive_discriminator(
                disc_reconstruct_x)
            gen_reconstruct_class_loss_x = self.class_loss_fn(label_x,
                                                              label_predicted_reconstruct_x)

            gen_same_disc_loss_x = self.generator_loss_deceive_discriminator(
                disc_same_x)
            gen_same_class_loss_x = self.class_loss_fn(label_x,
                                                       label_predicted_same_x)

            gen_fake_disc_loss_y = self.generator_loss_deceive_discriminator(
                disc_fake_y)
            gen_fake_class_loss_y = self.class_loss_fn(label_y,
                                                       label_predicted_fake_y)

            # Generator image loss
            reconstruct_image_loss_x = self.reconstruct_loss_fn(real_x,
                                                                reconstruct_x) * self.lambda_reconstruct
