...
def InceptionResNetV2(input_shape=None,
    ...
    x = conv3d_bn(img_input, block_size * 2, 3, strides=2, padding=padding,
                  activation=base_act,
                  name=f"{name_prefix}conv_down_1")
    x = conv3d_bn(x, block_size * 2, 3,
                  padding=padding, activation=base_act)
    x = conv3d_bn(x, block_size * 4, 3, activation=base_act)
    # x.shape: [B 128 128 64]
    x = layers.MaxPooling3D(3, strides=2, padding=padding,
                            name=f"{name_prefix}maxpool_1")(x)
    x = conv3d_bn(x, block_size * 5, 1, padding=padding, activation=base_act)
    x = conv3d_bn(x, block_size * 12, 3, padding=padding, activation=base_act)
    # x.shape: [B 64 64 192]
    x = layers.MaxPooling3D(3, strides=2, padding=padding,
                            name=f"{name_prefix}maxpool_2")(x)

    # Mixed 5b (Inception-A block): [B 64 64 320] or 35 x 35 x 320
    branch_0 = conv3d_bn(x, block_size * 6, 1, activation=base_act)
    branch_1 = conv3d_bn(x, block_size * 3, 1, activation=base_act)
    branch_1 = conv3d_bn(branch_1, block_size * 4, 5, activation=base_act)
    branch_2 = conv3d_bn(x, block_size * 4, 1, activation=base_act)
    branch_2 = conv3d_bn(branch_2, block_size * 6, 3, activation=base_act)
    branch_2 = conv3d_bn(branch_2, block_size * 6, 3, activation=base_act)
    branch_pool = layers.AveragePooling3D(3, strides=1, padding='same')(x)
    branch_pool = conv3d_bn(branch_pool, block_size *
    ...
    branch_0 = conv3d_bn(x, block_size * 24, 3, strides=2,
                         padding=padding, activation=base_act)
    branch_1 = conv3d_bn(x, block_size * 16, 1, activation=base_act)
    branch_1 = conv3d_bn(branch_1, block_size * 16, 3, activation=base_act)
    branch_1 = conv3d_bn(branch_1, block_size * 24, 3,
                         strides=2, padding=padding, activation=base_act)
    branch_pool = layers.MaxPooling3D(3, strides=2, padding=padding)(x)
    ...
    branch_0 = conv3d_bn(x, block_size * 16, 1, activation=base_act)
    branch_0 = conv3d_bn(branch_0, block_size * 24, 3,
                         strides=2, padding=padding, activation=base_act)
    branch_1 = conv3d_bn(x, block_size * 16, 1, activation=base_act)
    branch_1 = conv3d_bn(branch_1, block_size * 18, 3,
                         strides=2, padding=padding, activation=base_act)
    branch_2 = conv3d_bn(x, block_size * 16, 1, activation=base_act)
    branch_2 = conv3d_bn(branch_2, block_size * 18, 3, activation=base_act)
    branch_2 = conv3d_bn(branch_2, block_size * 20, 3,
                         strides=2, padding=padding, activation=base_act)
    branch_pool = layers.MaxPooling3D(3, strides=2, padding=padding)(x)
    branches = [branch_0, branch_1, branch_2, branch_pool]
    x = layers.Concatenate(
    ...
    x = conv3d_bn(x, block_size * 96, 1,
                  activation=last_act,
                  name=f'{name_prefix}conv_7b')

    # Create model.
    model = Model(
        img_input, x, name=f'{name_prefix}inception_resnet_v2')
...
def inception_resnet_block(x, scale, block_type, block_idx, block_size=16, activation='relu',
    ...
    if block_type == 'block35':
        branch_0 = conv3d_bn(x, block_size * 2, 1, activation=activation)
        branch_1 = conv3d_bn(x, block_size * 2, 1, activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 2,
                             3, activation=activation)
        branch_2 = conv3d_bn(x, block_size * 2, 1, activation=activation)
        branch_2 = conv3d_bn(branch_2, block_size * 3,
                             3, activation=activation)
        branch_2 = conv3d_bn(branch_2, block_size * 4,
                             3, activation=activation)
        branches = [branch_0, branch_1, branch_2]
    elif block_type == 'block17':
        branch_0 = conv3d_bn(x, block_size * 12, 1, activation=activation)
        branch_1 = conv3d_bn(x, block_size * 8, 1, activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 10,
                             [7, 1, 1], activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 11,
                             [1, 7, 1], activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 12,
                             [1, 1, 7], activation=activation)

        branches = [branch_0, branch_1]
    elif block_type == 'block8':
        branch_0 = conv3d_bn(x, block_size * 12, 1, activation=activation)
        branch_1 = conv3d_bn(x, block_size * 12, 1, activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 14,
                             [3, 1, 1], activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 15,
                             [1, 3, 1], activation=activation)
        branch_1 = conv3d_bn(branch_1, block_size * 16,
                             [1, 1, 3], activation=activation)
        branches = [branch_0, branch_1]
    else:
