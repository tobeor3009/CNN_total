...
class ATTGan(Model):
    ...
    def train_step(self, batch_data):
        ...
        with tf.GradientTape(persistent=True) as gen_tape:
            ...
            disc_fake_y, label_pred_fake_y = self.discriminator(fake_y)
            # custom
            disc_same_y, label_pred_same_y = self.discriminator(fake_y)

            # Generator class loss
            gen_fake_y_class_loss = self.class_loss_fn(label_y,
                                                       label_pred_fake_y)
            gen_same_y_class_loss = self.class_loss_fn(label_y,
                                                       label_pred_same_y)
            gen_class_loss = gen_fake_y_class_loss + gen_same_y_class_loss
            # Generator adverserial loss
            gen_fake_y_disc_loss = to_real_loss(disc_fake_y)
            gen_same_y_disc_loss = to_real_loss(disc_same_y)
            gen_disc_loss = gen_fake_y_disc_loss + gen_same_y_disc_loss
